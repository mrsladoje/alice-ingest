...
- **History indices** — overlapping **`(end_time, id)`** watermark, PIT + `search_after`, deterministic upserts, dedup on ingest.

**The cursor sorts on the `id` source field, not `_id`.** The alert mapping declares `id` as a `keyword` with doc values, and `AlertMover` copies each hit into history with `.id(hit.id)` — **the same identity survives the move**, which is the real reason to use it: an `_id`-based cursor would mint separate current and history signals for one alert. Contract assertion at ingest: `_source.id == hit._id`, and `id` non-empty.

A nuance worth recording, because it changes the *reason* rather than the decision: sorting on `_id` is not actually prohibited here — `indices.id_field_data.enabled` defaults to **true** in OpenSearch, so an `_id` sort runs and merely warns. It is still wrong for us on two counts. It loads every `_id` into fielddata on a **1 GB heap**, and it is a *dynamic cluster setting* someone can turn off, which would convert a working cursor into a runtime failure with no code change. Identity continuity is the argument; heap and fragility are the corroboration.
