...
**The next live deploy exposed three independent convergence failures.** A newly indexed OpenSearch alert can validly have an empty `_source.id` while its generated hit `_id` is already durable; identity now falls back to that hit ID and still rejects a genuine mismatch. The projector retained the complete two-hour anomaly result set, reached 1.4 GB and repeatedly lost its two-minute PIT or was OOM-killed; it now consumes event-ordered pages into shared episode timelines, writes bounded deterministic bulks, uses a ten-minute renewable PIT lease, and advances watermarks only after the whole traversal. Finally, the deployed ops actions had already moved to background jobs but two contracts still asserted the retired synchronous redirect flow. Both now exercise the action table and persistent job result, and `make deploy` runs `make contract` before asking for a vault password or touching a VM.

**Offline proof that the hardest contracts hold.** `deploy/roles/dashboards/files/test_signal_contract.py` runs without a cluster, in the deploy and via `make contract`, and asserts: one logical alert stays one signal row across the current → history move; every canonical label is present with an explicit value on both lanes; an EPN's parent comes from the roster and fails closed to the sentinel when unassigned; an episode opens on a breach, needs K explicit healthy windows to resolve, goes STALE rather than RESOLVED on a missing evaluation, and closes as expected when the roster retires the entity. It also covers empty source-ID fallback with mismatch rejection, bounded page consumption and bulk writes, state continuity across page boundaries, exact-ID recovery of preserved-clock terminal episodes, no watermark advance after an interrupted traversal, duplicate evidence, repeated-firing idempotence, active-plus-old-history ordering in both scan orders, separate episode generations, notification linkage, pipeline composition, refresh-safe background ops actions, resolved-page accountability, the restricted break-glass contract, the post-restart catch-up barrier, the post-collector heartbeat gate, and immutable detector migration — 39 assertions. The episode-key contracts were explicitly checked against deliberate mutations; two early attempts passed under mutation and were rewritten until they did not, which is the only reason those defects were caught rather than shipped.
