...
| **Severity mix** | Built-in distribution per producer ~**90% info / 8% warn / 1.5% error / 0.5% debug** — so `application-logs-central` populates and the error-rate alert has fuel. InfoLogger error lines carry the rich fields. |
| **Classification** | At the **collector** (Fluent Bit). `source == infologger → infologger`; else by severity: `info → application-logs-local`, `warn/error/debug → application-logs-central`. (Index families are by severity/access-pattern, **not** by producer type.) |
...
| **OpenSearch** | **3-node cluster**, role-tagged via shard allocation awareness: **1× `worker`** (holds `application-logs-local`/`-other`), **2× `storage`** (hold `infologger`; replica lands on the 2nd storage node → genuinely demonstrates *dedicated storage + higher replication*). All 3 cluster-manager-eligible (quorum 3). Heap-capped ~2 g each. **No co-location** with producer nodes. |
| **Index lifecycle (ISM)** | Per-family: `application-logs-local` → strong compression, short retention; `application-logs-central` → strong compression, long retention; `infologger` → fast access, higher replication. (Exact rollover/retention numbers left to implementation — already hands-on with ISM.) |
