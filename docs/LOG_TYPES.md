...
`deploy/roles/loggy_collector/templates/`. Every source the collector reads has a row
...
`deploy/roles/loggy_collector`. Informational lines stay on the node, so a
