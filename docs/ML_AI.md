...
2. **Layer 1 — per-failure-mode RCF log detectors** over the index families (`infologger`, `application-logs-central`, `application-logs-local`), **category field `host` (EPN)** — not `node`, which is the collector. Small feature sets (≤5, per OpenSearch guidance): volume (zero-imputed), E/F count, latency. Explicit per-detector interval, window delay ≥ measured p99 ingestion lag, and per-feature imputation policy.
