...
- **Cluster:** 5× m2.medium (2 vCPU / 3.75 GB). 2 workers (`node-01/02`: data+ingest, box-pinned `application-logs-local-<node_id>`), 3 storage (`alice-ingest-3/4/5`: manager+data+ingest, hold `infologger` + `application-logs-central` + `cockpit-metrics`). OpenSearch 3.7.0 RPM, security disabled, heap **1g** (Stage 1).
...
- **Provisioning pattern:** `deploy/roles/sweet_os_dashboards/tasks/main.yml`, `deploy/roles/alerting_monitors/tasks/main.yml` and `deploy/roles/anomaly_detection/tasks/main.yml` run rendered `sh`/`curl` scripts on the control host (`templates.sh` → `patterns.sh` → ndjson import → hydrate), idempotent via PUT/ensure, verified by assertions. New AD/alerting provisioning slots into exactly this pattern.
...
- **the nil guard on `rewrite_tag` re-emitted records.** `family.local` / `family.central` records pass the `match: '*'` stamp a second time; they should hit `record['collector_time'] ~= nil` and return code 0. If the emitter strips the field instead, those two families get an accept time ~ms later than the truth (harmless) — but if the guard misfires the other way, the field goes missing and every application-logs family detector starves.
...
| `central-per-epn` | `application-logs-central` | `host` | volume, error count (`Error`/`Fatal`/`err`) | YES | 1 min |
| `central-per-epn-slow` | `application-logs-central` | `host` | same | YES | 30 min |
| `local-volume` | `application-logs-local-*` | `host` | volume only | YES | 1 min |
| `local-volume-slow` | `application-logs-local-*` | `host` | same | YES | 30 min |
| `local-per-epn-entry-lag` | `application-logs-local-*` | `host` | `avg(enter_system_lag_ms)` only | NO | 1 min |
| `local-per-epn-entry-lag-slow` | `application-logs-local-*` | `host` | same | NO | 30 min |
| `local-collector-shipping-lag` | `application-logs-local-*` | `node` | `avg(ingest_lag_ms)` only | NO | 1 min |
| `local-collector-shipping-lag-slow` | `application-logs-local-*` | `node` | same | NO | 30 min |
...
| `trend-central-volume` | `application-logs-central` | `host` | share of fleet volume |
| `trend-central-errors` | `application-logs-central` | `host` | error share of own volume |
| `trend-local-volume` | `application-logs-local-*` | `host` | share of fleet volume |
| `trend-local-entry-lag` | `application-logs-local-*` | `origin_host` | p95 `enter_system_lag_ms` (self-gating via ceiling) |
| `trend-local-shipping-lag` | `application-logs-local-*` | `node` | p95 `ingest_lag_ms` |
...
3. **Category field corrected per family:** the doc says "category field `host`" — that's right for `application-logs-`* but **wrong for** `infologger`**, whose mapping has** `hostname`, and the strict mapping means a `host` category there would simply fail. Plan uses `hostname`/`host` per family.
