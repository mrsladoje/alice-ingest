# Ansible Role: `loggy_alertmanager`
...
the same machine. nginx from `loggy_os_dashboards` publishes the web interface
...
- **Handlers are flushed inside the role.** `loggy_signal_projector` asserts
...
- **`inhibit_rules` is generated, never written.** `files/causal_edges.json`
  is a copy of the file `loggy_signal_projector` owns, and the signal contract
...
cluster. `loggy_signal_projector` must run its receiver entry point on this
...
`/-/ready` answers. `loggy_os_dashboards` owns the nginx virtual host that
