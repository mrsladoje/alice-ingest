...
| `stamper` | workers | Stamps every record with its template identity before it reaches OpenSearch. One long-running service per worker, in-band between Fluent Bit's filters and its outputs. Installed before `collector`, so its socket exists when Fluent Bit starts. |
...
| `template_catalog` | one worker | The central maintenance of the template catalog: definition expiry, query-history expiry and the two counting checks. One oneshot unit on a timer, on the host named by `template_catalog_maintenance_host`. |
...
5. **Ingest last of all.** `stamper` then `collector` in one play — the socket
   must exist before Fluent Bit starts — then `template_catalog`, `producer`
   and `faults`. The firehose starts only once everything that reads it exists.
