...
LOCAL_INDEX_TEMPLATE="${ALICE_LOCAL_INDEX_TEMPLATE:-/etc/loggy/local-index-template.json}"
...
if [ -z "$NODE" ]; then
  ...
  echo "[register-node] FATAL: Ansible writes it into /etc/loggy/node.env" >&2
...
# The mapping is not defined here. loggy_opensearch renders one file per worker
...
index_template_body() {
  if [ ! -f "$LOCAL_INDEX_TEMPLATE" ]; then
    ...
    echo "[register-node] FATAL: the loggy_opensearch role installs it. Creating" >&2
