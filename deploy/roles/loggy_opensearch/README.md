...
```
 play 1  hosts: alice_nodes        loggy_opensearch, install
 ...
 play 3  hosts: control            loggy_opensearch, opensearch_configure_cluster: true
...
└────────────────────────────────────┬───────────────────────────────────────┘
                                     v
┌─ 2. SHARED FILES ──────────────────────────────────────────────────────────┐
│  /etc/loggy                 0755, root — shared with the collector         │
...
the default and is the site value, shared with the `loggy_os_dashboards` role so both
...
```yaml
opensearch_node_env_dir: /etc/loggy
opensearch_node_env_file: "{{ opensearch_node_env_dir }}/opensearch-node.env"
opensearch_register_script: /opt/loggy/register_node.sh
opensearch_register_script_dir: /opt/loggy
...
Files shared with the collector. `loggy_collector` loads the env file and names
...
opensearch_cluster_config_root: /opt/loggy/init
...
- **`opensearch_cluster_config_root` is shared with the alice-service roles.**
  ...
  because `loggy_anomaly_detection` and `loggy_signal_projector` stage a
...
- **`opensearch_version` is shared with `loggy_os_dashboards`.** Change it in
  `group_vars`.
...
- **`/etc/loggy` has no single owner.** This role and `loggy_collector`
