# Ansible Role: sweet_shifter_view
...
│  /opt/sweet/shared/template_contract.py       the contract, vendored        │
│  /opt/sweet/templating/{drainbench,masking}.py  the frozen mining recipe    │
│  /opt/sweet/{shifter,templates_view,semantic,triage}.py   the server        │
│  /opt/sweet/shifter-venv   python3.13..3.10 + drain3 0.9.11 + model2vec     │
│      no interpreter of 3.10 or newer on the host: the play fails here       │
│  /opt/sweet/models/potion-retrieval-32M   fetched once, pinned revision     │
│  /opt/sweet/live/   shifter.js templates.js shifter.css shrunk on the       │
...
`sweet_opensearch` must have run in both of its modes first: the query lane,
...
`template_contract.py`; `sweet_collector` and `sweet_template_catalog` ship
...
model once. Run this play before `sweet_collector`; the two proofs at the end
...
- `shifter_port` and `shifter_ingest_path` are written into the collector's
  output by `sweet_collector`; change them in `group_vars`, never here alone.
