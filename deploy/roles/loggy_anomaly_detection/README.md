# Ansible Role: loggy_anomaly_detection
...
│  backtest.py  --> /opt/loggy                     for playbooks/backtest   │
└───────────────────────────────────────────────────────────────────────────┘
                                     │
                                     v
...
`loggy_opensearch` must have run on the control host in its configure-the-
...
`loggy_cockpit_metrics` must be running its
...
reads it at build time. `loggy_signal_projector` carries a copy.
...
|---|---|---|
| `verify_detection.py` | this role, then `loggy_cockpit_metrics` after the collectors are up, then `loggy_signal_projector` | Asserts the whole detection layer against the cluster. Exit 1 fails the deploy. |
