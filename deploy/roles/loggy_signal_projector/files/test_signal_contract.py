...
# On the control host every module below is staged flat in /opt/loggy,
...
for _sibling in ("alice_ops", "loggy_anomaly_detection",
                 "loggy_cockpit_metrics", "loggy_trend_rollup"):
...
def test_cockpit_headlines_episodes_not_raw_detector_exhaust():
    generator = _checkout_file(
        "roles", "loggy_os_dashboards", "files", "gen_cockpit.py")
    if not generator:
    ...
    try:
        verify.COCKPIT_NDJSON = _checkout_file(
            "roles", "loggy_os_dashboards", "files", "cockpit.ndjson")
...
def test_bucket_key_gate_runs_only_after_the_projector_restart():
    ...
    verifier = _checkout_file(
        "roles", "loggy_anomaly_detection", "files", "verify_detection.py")
    if verifier is None:
        print("[signal-contract] "
...
def test_the_deploy_gate_fails_on_a_board_query_opensearch_rejects():
    fixture = _checkout_file("roles", "loggy_os_dashboards", "files", "cockpit.ndjson")
    if not fixture:
        print("[signal-contract] "
...
def test_every_local_import_survives_the_on_vm_layout():
    source = _checkout_file(
        "roles", "loggy_signal_projector", "files", "test_signal_contract.py")
    ...
    for declaration in declarations:
        with open(declaration) as fh:
            beside |= set(re.findall(
                r":\s*/opt/loggy/(\w+)\.py\s*$", fh.read(), re.M))
    ...
    check(not stranded,
          ...
          f"in /opt/loggy, so this file cannot even load on the control "
...
def test_episode_grouping_gate_runs_after_the_projector_upgrade():
    detection_path = _checkout_file(
        "roles", "loggy_anomaly_detection", "tasks", "detection.yml")
    control_path = _checkout_file(
        "roles", "loggy_signal_projector", "tasks", "control.yml")
    ...
    check("CHECK_EPISODE_GROUPING" not in detection,
          ...
          "the loggy_signal_projector role installs the projector that writes "
...
def test_push_heartbeat_gate_runs_after_collector_cutover():
    ...
    post_path = _checkout_file(
        "roles", "loggy_cockpit_metrics", "tasks", "post_collector.yml")
...
def test_detector_category_migration_recreates_instead_of_updating():
    ...
    for _ in range(7):
        candidate = os.path.join(
            here, "roles", "loggy_anomaly_detection", "templates",
            "detectors.sh.j2")
...
def test_projector_runtime_is_off_the_control_host():
    ...
    check(projector_play is not None
          ...
          and "- loggy_signal_projector" in projector_play.group(0),
          "the loggy_signal_projector role no longer runs in a play targeting the "
...
def test_dynamic_services_can_read_the_signal_catalog():
    ...
    if path is None:
        print("[signal-contract] "
              ...
              "loggy_signal_projector not beside this checkout")
        return
...
def test_deploy_preserves_the_replay_runtime_dropin():
    ...
    replay_role_path = _checkout_file(
        "roles", "loggy_replay", "tasks", "main.yml")
    replay_path = _checkout_file("playbooks", "replay.yml")
    unit_path = _checkout_file(
        "roles", "loggy_replay", "templates", "replay.service.j2")
...
VENDORED_SIGNAL_FILES = {
    "os_cursor.py": ("loggy_signal_projector", "loggy_anomaly_detection",
                     "loggy_cockpit_metrics", "alice_ops"),
    "signal_identity.py": ("loggy_signal_projector",
                           "loggy_anomaly_detection"),
    "signal_catalog.json": ("loggy_anomaly_detection",
                            "loggy_signal_projector"),
    "causal_edges.json": ("loggy_signal_projector", "alice_ops",
                          "loggy_alertmanager"),
}
...
def test_vendored_signal_files_are_byte_identical():
    ...
    for role in ("loggy_signal_projector", "loggy_anomaly_detection",
                 "loggy_cockpit_metrics", "alice_ops", "loggy_alertmanager"):
...
def test_the_registration_script_has_exactly_one_definition():
    ...
    # Each worker runs this as ExecStartPre and loggy_opensearch installs it
    # there. Two copies means two competing definitions.
    check([rel for rel, _ in scripts] ==
          [os.path.join("loggy_opensearch", "files", "register_node.sh")],
          "register_node.sh must exist exactly once, inside loggy_opensearch, "
    ...
    check([rel for rel, _ in mappings] ==
          [os.path.join("loggy_opensearch", "templates", "schema-per-worker",
    ...
    check("application-logs-local-%s-*" not in script_body,
          ...
          "read the rendered file loggy_opensearch installs beside it")
...
def test_status_exposes_functional_projector_and_replay_health():
    ...
    probe_path = _checkout_file(
        "roles", "loggy_anomaly_detection", "files", "detection_status.py")
