# Ansible Role: `loggy_signal_projector`
...
┌─ 1. INSTALL ──────────────────────────────────────────────────────────────┐
│  signal_projector.py                /opt/loggy   0755 root:root           │
...
│  test_signal_contract.py     /opt/loggy   0755 root:root                  │
...
The role stages its own inputs. It creates `/opt/loggy` and `/opt/loggy/init`,
...
are owned here; `signal_catalog.json` is owned by `loggy_anomaly_detection`.
`alice_ops`, `loggy_alertmanager`, `loggy_anomaly_detection` and
`loggy_cockpit_metrics` carry copies, and the contract test fails when any copy
differs from its source.

`loggy_opensearch` must have created the signals, incidents, notifications and
lane-state indices; a first cycle against a missing index creates it with dynamic
mappings the cockpit queries cannot use. `loggy_anomaly_detection` must have
loaded the monitors, detectors and forecasters the projector normalizes, and
staged `verify_detection.py` on the control host. `loggy_alertmanager` must be running
