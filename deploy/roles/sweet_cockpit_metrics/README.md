# Ansible Role: sweet_cockpit_metrics
...
└──────────────────────────────────────┬─────────────────────────────────────────┘
                                       v
┌─ VERIFY (verify_detection.py, staged by sweet_anomaly_detection) ──────────────┐
...
- `heartbeat_grace_seconds` and `cockpit_metrics_interval_seconds` change
  ...
  absence.
- A worker that runs `sweet_collector` but is missing from
