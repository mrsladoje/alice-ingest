# Ansible Role: loggy_os_dashboards
...
`loggy_opensearch` must have run its cluster-configure mode on the same host
...
- The bootstrap directory is shared with `loggy_opensearch` and the roles that
  stage their scripts beside these. Both declarations of `alice_bootstrap_root`
  must stay `/opt/loggy/init`.
