...
ISM does the part it can: `loggy_opensearch` age-deletes the
`template-buckets-5m-*` and `template-buckets-1h-*` indices, which are the
large, growing part of this data.

## Requirements

`loggy_opensearch` must have run in both its modes first: install on the
...
with their mappings and retention. `loggy_collector` must run on the workers,
