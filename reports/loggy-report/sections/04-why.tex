...
The stamper runs beside the collector, so no raw line leaves its machine. It runs drain3~\cite{ref:drain3}, because no collector filter language can host it. Without ground truth, cost chose the parser: PIPLUP, another log-template parser, costs 2.56~times drain3 against a gate of plus or minus 25~\%. The masker, which replaces variable parts of a line with placeholders, was 74 to 87~\% of the mining cost, so we rewrote it. Masking fell from 11.93 to 1.82 core-seconds per million lines on InfoLogger, one core-second being one core busy for one second. It fell as far on the process tree and on DDS, and no template changed. Mining the parser corpus of 45,596,613 lines now costs 8.33 core-seconds per million. Those figures price the miner on a stored corpus. What the stamper costs beside a running collector is not measured.

\paragraph{Detection on numbers, not on text}\label{sec:why-detection}
Three lanes read numbers about the logs inside the cluster, and Random Cut Forest detectors~\cite{ref:rcf} read a silent host as zero records. Deep sequence models score about 0.23 to 0.27 on F1 once the logs drift from the data they learnt on. F1 weighs faults caught against false alarms, and 1 is perfect. Plain principal component analysis scores about 0.71 on the same logs. One route on the log text stays open: mine the templates, then run nearest-neighbour search on those. Search on raw messages lost on cost, and templating as the detector lost because the signal runs out.
