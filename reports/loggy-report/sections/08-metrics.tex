...
Two store plugins do the work, and they are not the same thing. A \textbf{detector} (Anomaly Detection) grades how surprising an interval is. A \textbf{monitor} (Alerting) runs a scheduled search and fires when a condition is true. \textbf{A detector writes a grade. A monitor decides whether anyone is paged.} Figure~\ref{fig:alerts} in Section~\ref{sec:design-watch} shows the whole path, from the numbers to one alert.

\subsection{What feeds it}\label{sec:metrics-inputs}

We knew from the start what matters on a worker: \textbf{volume}, \textbf{errors} and \textbf{health}. Two indices hold them.
...
Both are read as \textbf{p95, never the mean}. A backlog shows in the slowest records before it moves the average.

\textbf{The metrics index} has two writers, and \textbf{nothing reaches into a worker to scrape it}:

\begin{itemize}
\item \textbf{Each collector pushes} its own sample every 30~seconds: alive, records out, errors, retries.
\item \textbf{The poller} samples the cluster and Dashboards every 30~seconds, plus a roster check: which collectors were expected to speak and did not.
\end{itemize}
...
A detector does not read log lines. Every interval it runs a few aggregations, split per entity, and each result becomes a point in a small feature space: volume, error count, p95 lag, heap. A Random Cut Forest \cite{ref:rcf} then slices that point cloud at random until every point is isolated. \textbf{A point in a crowd needs many cuts, a loner needs one or two, and few cuts means a high grade.} Many trees vote, so the grade is stable. The properties that matter here:
...
\item \textbf{Multivariate.} It flags \textbf{combinations that never happened}, such as throughput collapsing while retries climb, or heap rising while indexing falls.
...
\textbf{Same failure, two horizons.} Every log detector runs twice: at \textbf{one minute}, where eight intervals give eight minutes of context, and at \textbf{30 minutes}, where they give four hours. The slow twin has the same features. It catches a change the fast model has already learnt as normal. Table~\ref{tab:detectors} lists all seventeen.
...
\code{il-per-epn} & EPN & InfoLogger volume and error count: one EPN silent or erroring while the farm looks fine \\
...
\code{il-per-epn-entry-lag} & EPN & p95 entry lag on InfoLogger \\
...
\code{il-collector-shipping-lag} & collector & p95 shipping lag on InfoLogger \\
\code{local-collector-shipping-lag} & collector & p95 shipping lag on local info lines \\
\bottomrule
\end{tabularx}
\end{table}

Shipping lag is a collector problem, not an EPN problem, so those two are keyed on the collector. One \textbf{forecaster}, from the same family, predicts each store node's disk fill a day ahead from a week of hourly points. Disk is the one metric with a real cliff and a smooth approach.

\textbf{A detector never pages.} It writes a grade from 0 to 1 and a confidence into the results index. The monitor \code{ad-high-grade} turns that into an alert when both pass 0.7. A grade is surprise, not harm.

\subsection{Monitors: the cliffs we know}\label{sec:metrics-monitors}

A monitor is a \textbf{scheduled job inside the cluster}. Every minute, or every ten, the store runs the monitor's search, hands the result to a short script, and fires if the script says yes. Nothing is trained: the script is a threshold, a ratio or a presence test. There are two kinds.
...
\code{telemetry-silence} & no cluster or Dashboards sample for 5 minutes & the poller is dead \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Static rules catch cliffs. The forest catches unfamiliar shapes. Neither catches a slow drift}, because the forest learns it as normal. The trend lane compares against a fixed week instead.

It rests on our own \textbf{trend rollup} service:

\begin{enumerate}
\item Every ten minutes it writes \textbf{one small row per host and family}: records, errors and fatals, and the p95 and mean of both lags.
...
\item A host that logged in the last day and is silent now gets a \textbf{zero row}. The store's own rollup writes nothing for silence, which is why this one is ours.
\item A commit row closes each slice, so no rule reads half a slice.
\end{enumerate}

Nine rules read those rows. Each compares \textbf{three consecutive ten-minute slices} with the \textbf{previous seven days}, leaving out the last 40 minutes so the slices are not part of their own baseline. It fires only when all three slices agree (Table~\ref{tab:trendrules}).
...
\midrule
Volume & the host's \textbf{share of the fleet} & twice the week's share, or half of it; a run start moves every host and cancels \\
Errors & errors as a \textbf{share of the host's own records} & twice the week's share; doubling traffic and errors together stays quiet \\
...
\item \textbf{Replay lies about entry lag.} The replay keeps the original timestamps, so entry lag is the archive's age, about a month. Shipping lag stays real on replay.
...
\textbf{In one sentence: the projector turns every alert and every grade into signals and episodes that are stored, deduplicated and have a start and an end.}

Thirty monitors and seventeen detectors write to different indices, and a firing monitor writes a new row every minute. Alertmanager \cite{ref:alertmanager} groups and routes well, but it keeps nothing: it forgets an alert five minutes after the last post. Every 30~seconds the projector reads all of it and writes:

\begin{itemize}
\item \textbf{Signals}: one index for every alert and grade, firing and cleared, named from one catalogue.
\item \textbf{Episodes}: all signals of one alert name on one entity become one incident with a \textbf{start and an end}. It goes open, recovering, resolved. It closes after one healthy window for a hard rule, three for a trend rule or detector. A missing evaluation is never a recovery, so a lane that stops reporting leaves its episode open and marked stale. When half the roster goes silent at once, one fleet incident opens instead of hundreds.
\item \textbf{Re-posts}: every open episode goes to Alertmanager on every cycle, well inside its five-minute memory.
\end{itemize}

\begin{table}[htbp]\caption{The projector remembers and deduplicates, and Alertmanager decides when a person is told.}\label{tab:projector}\centering\footnotesize
\begin{tabularx}{\textwidth}{L{0.24\textwidth} Y Y}
\toprule
 & Signal projector & Alertmanager \\
\midrule
...
\textbf{Why it needed a watermark.} Its sources arrive late and out of order: a grade lands after its window closes, a history row when its alert ends, and the projector itself restarts. So it keeps one \textbf{watermark} per source, the last moment it fully processed:
...
\textbf{Who watches the watchers.} Every monitor runs inside the cluster, so a dead cluster silences all of them at once, and silence reads as health. Table~\ref{tab:watchdogs} gives the checks on the detection machinery itself.
...
\midrule
\code{opensearch-unreachable} & the projector fails two cluster probes in a row & raised by the projector straight to Alertmanager; the one alert that survives a dead cluster \\
...
\code{alertmanager-down} & the projector saw Alertmanager unreachable for 5 minutes & break-glass: nobody is receiving pages \\
